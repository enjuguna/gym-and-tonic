    ...
    <div className="grain min-h-screen bg-[var(--paper)] text-[var(--ink)]">
      <header className="mx-auto flex max-w-6xl items-center justify-between px-5 py-5 sm:px-8">
        ...
        <nav className="hidden items-center gap-6 text-sm text-[var(--muted)] sm:flex" aria-label="Main navigation">
          ...
          <a href="/today" onClick={openPlanner} className="button-primary">Open Today</a>
        </nav>
      ...
      <main>
        <section className="mx-auto grid max-w-6xl gap-8 px-5 pb-16 pt-8 sm:px-8 lg:grid-cols-[1.05fr_.95fr] lg:items-center lg:py-16">
          <div className="max-w-xl">
            <p className="eyebrow text-[var(--terra)]">A training rhythm, made personal</p>
            <h1 className="mt-4 font-display text-5xl font-semibold leading-[.96] tracking-tight sm:text-6xl">Your week, well trained.</h1>
            <p className="mt-6 max-w-lg text-base leading-7 text-[var(--muted)]">Gym &amp; Tonic is a calm space to plan a session, follow it properly, and see the small proof that you are showing up.</p>
            <div className="mt-8 flex flex-wrap gap-3"><a href="/today" onClick={openPlanner} className="button-primary text-sm">Open Today <span aria-hidden="true">→</span></a><a href="#how-it-works" className="button-secondary text-sm">See the flow</a></div>
        ...
        <section className="mx-auto grid max-w-6xl gap-6 px-5 py-16 sm:px-8 lg:grid-cols-2">
          ...
          <div className="flex flex-col justify-center py-3 lg:px-8"><p className="eyebrow text-[var(--terra)]">Food for real life</p><h2 className="mt-3 font-display text-4xl font-semibold tracking-tight">The details should feel familiar.</h2><p className="mt-4 max-w-md leading-7 text-[var(--muted)]">Sessions can be paired with practical meal ideas from different food traditions, without turning food into a prescription.</p><a href="/meals" onClick={openPlanner} className="button-quiet mt-5 w-fit">Explore meal ideas →</a></div>
        </section>
