...
export function ReflectionPanel({ entry, onSave, onClose }: Props) {
  ...
  useEffect(() => { const previous = document.activeElement as HTMLElement | null; headingRef.current?.focus(); const onKey = (event: KeyboardEvent) => { if (event.key === "Escape") onClose(); }; window.addEventListener("keydown", onKey); return () => { window.removeEventListener("keydown", onKey); previous?.focus(); }; }, [onClose]);
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-3 sm:items-center" onClick={onClose}>
      <div ref={dialogRef} className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="reflection-heading" onClick={(event) => event.stopPropagation()}>
        <p className="eyebrow text-emerald-700">Nice work</p><h2 ref={headingRef} tabIndex={-1} id="reflection-heading" className="mt-1 font-serif text-2xl font-semibold outline-none">How did it feel?</h2><p className="mt-1 text-sm text-stone-500">Optional — a tiny note helps your next week get smarter.</p>
