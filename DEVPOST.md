...
## What it does
Gym & Tonic is a Kenyan-rooted weekly training planner (Mon–Sun × AM/PM = 14 session slots). Open it in ChatGPT's built-in browser or Chrome with WebMCP and your AI coach can:
...
The player experience is editorial, not dashboard-y: photo heroes with slow zoom, a coach voice that reacts to every action ("Wheel Day on the board. Your future self just exhaled."), DayStory spreads with numbered exercises and post-workout kitchen cards (ugali na ndengu, mukimo on leg day), a muscle-group coverage strip that lights up as groups get planned, and a rotated rubber stamp at 14/14: **"WEEK COMPLETE · WELL TRAINED."**

## How we built it
Astro static site + React island; Zustand store where every agent write funnels through a single `applyProposal()` pipeline (the trust pattern high-stakes domains need); pure-function domain layer (exercise catalogue, gear aggregation, balance analysis, non-repeating session generator); 9 tools registered via `document.modelContext.registerTool()` with JSON-Schema inputs and honest `readOnlyHint` annotations; Fraunces/Inter typography, curated Kenya-first photography with gradient fallbacks, procedural Web Audio stings. Fully client-side, no backend, no accounts.
...
2. **Representation matters.** Generic stock photos showed European gyms to a Kenyan audience. We audited every image visually and swapped in verified African/Kenyan scenes — including ugali being served, verified by the photographer's own framing.
...
- Kenyan identity through places and culture — Karura runs, Kasarani quiet stands, Fisi Club energy, refuels from home — not token imagery.
